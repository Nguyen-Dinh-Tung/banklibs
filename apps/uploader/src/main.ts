...
async function bootstrap() {
  const app = await NestFactory.create<NestExpressApplication>(UploaderModule);
  ...
  await app.listen(configService.get<number>('UPLOADER_PORT'));

  Logger.log(
    `SERVER RUNING PORT : ${configService.get<number>('UPLOADER_PORT')}`,
  );
}
bootstrap();
